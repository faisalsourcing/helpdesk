...
class ChannelEmailAccountWizard extends App.ControllerWizardModal
  ...
  probeInboundMessagesFound: (data, verify) =>
    message = App.i18n.translateContent('We have already found %s email(s) in your mailbox. We will move them all from your mailbox into devsquest.', data.content_messages)
  ...
  probeInboundArchive: (data) =>
    ...
    message = App.i18n.translateContent('In addition, we have found emails in your mailbox that are older than %s weeks. You can import such emails as an "archive", which means that no notifications are sent and the tickets have the status "closed". However, you can find them in devsquest anytime using the search function.', data.archive_week_range)
