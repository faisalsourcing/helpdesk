  ...
  @defaults: ->
    groups =
      ...
      article:
        ...
        options: [
          {
            value:    'x-devsquest-article-internal'
            name:     __('Internal')
            options:  { true: 'yes', false: 'no'}
          }
          {
            value:    'x-devsquest-article-type_id'
            name:     __('Type')
            relation: 'TicketArticleType'
          }
          {
            value:    'x-devsquest-article-sender_id'
            name:     __('Sender')
        ...
        for row in App[groupMeta.model].configure_attributes
          ...
          if row.type isnt 'password' && row.name.substr(row.name.length-4,4) isnt '_ids'
            ...
            if !row.readonly
              ...
              elements["x-devsquest-ticket-#{config.name}"] = config
    ...
    for item in groups.ticket.options
      item.value = "x-devsquest-ticket-#{item.value}"
