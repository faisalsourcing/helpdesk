...
module ApplicationModel::CanQueryCaseInsensitiveWhereOrSql
  ...
  included do
    ...
    #
    # @example
    #  Organization.where_or_cis(%i[name note], "%devsquest%").to_sql
    #  #=> "SELECT `organizations`.* FROM `organizations` WHERE (`organizations`.`name` LIKE '%devsquest%' OR `organizations`.`note` LIKE '%devsquest%')"
    #
