    ...
    message_ids.each do |message_id|
      ...
      timeout(FETCH_METADATA_TIMEOUT) do
        ...
        notice += <<~NOTICE
          ...
          to prevent devsquest from trying to import it again.
        NOTICE
        Rails.logger.error "Net::IMAP failed to parse message #{message_id}: #{e.message} (#{e.class})"
        Rails.logger.error '(See https://github.com/devsquest/devsquest/issues/2754 for more details)'
      end
...
=end
  ...
  def messages_is_too_old_verify?(message_meta, count, count_all)
    ...
    return true if headers['X-devsquest-Verify-Time'].blank?

    begin
      verify_time = Time.zone.parse(headers['X-devsquest-Verify-Time'])
    rescue => e
      Rails.logger.error e
      return true
    end
  ...
  def messages_is_verify_message?(headers)
    return true if headers['X-devsquest-Verify'] == 'true'

    false
  end

  def messages_is_ignore_message?(headers)
    return true if headers['X-devsquest-Ignore'] == 'true'
