  ...
  def fetch(options, channel, check_type = '', verify_string = '')
    ...
    if check_type == 'check'
      ...
      mails.each do |m|
        ...
        if !mail.match?(%r{(X-devsquest-Ignore: true|X-devsquest-Verify: true)})
          content_messages += 1
    ...
    mails.first(2000).each do |m|
      ...
      if mail.match?(%r{(X-devsquest-Ignore: true|X-devsquest-Verify: true)}) && mail =~ %r{X-devsquest-Verify-Time:\s(.+?)\s}
        begin
