...
=begin
  ...
  mail = {
    ...
    # ignore email header
    x-devsquest-ignore: 'false',

    # customer headers
    x-devsquest-customer-login:     '',
    x-devsquest-customer-email:     '',
    x-devsquest-customer-firstname: '',
    x-devsquest-customer-lastname:  '',

    # ticket headers (for new tickets)
    x-devsquest-ticket-group:    'some_group',
    x-devsquest-ticket-state:    'some_state',
    x-devsquest-ticket-priority: 'some_priority',
    x-devsquest-ticket-owner:    'some_owner_login',

    # ticket headers (for existing tickets)
    x-devsquest-ticket-followup-group:    'some_group',
    x-devsquest-ticket-followup-state:    'some_state',
    x-devsquest-ticket-followup-priority: 'some_priority',
    x-devsquest-ticket-followup-owner:    'some_owner_login',

    # article headers
    x-devsquest-article-internal: false,
    x-devsquest-article-type:     'agent',
    x-devsquest-article-sender:   'customer',
...
=end
  ...
  def process(channel, msg, exception = true)
    ...
    message = "Can't process email, you will find it for bug reporting under #{filename}, please create an issue at https://github.com/devsquest/devsquest/issues"
  ...
  def _process(channel, msg)
    ...
    # check ignore header
    if mail[:'x-devsquest-ignore'] == 'true' || mail[:'x-devsquest-ignore'] == true
      Rails.logger.info "ignored email with msgid '#{mail[:message_id]}' from '#{mail[:from]}' because of x-devsquest-ignore header"
      return
    end
    ...
    Transaction.execute(transaction_params) do
      ...
      # get sender user
      session_user_id = mail[:'x-devsquest-session-user-id']
      if !session_user_id
        raise __('No x-devsquest-session-user-id, no sender set!')
      end

      session_user = User.lookup(id: session_user_id)
      if !session_user
        raise "No user found for x-devsquest-session-user-id: #{session_user_id}!"
      end
      ...
      if mail[:'x-devsquest-ticket-id']
        ticket = Ticket.find_by(id: mail[:'x-devsquest-ticket-id'])
      end
      if mail[:'x-devsquest-ticket-number']
        ticket = Ticket.find_by(number: mail[:'x-devsquest-ticket-number'])
      end
      ...
      if ticket
        ...
        # save changes set by x-devsquest-ticket-followup-* headers
        ticket.save! if ticket.has_changes_to_save?

        # set ticket to open again or keep create state
        if !mail[:'x-devsquest-ticket-followup-state'] && !mail[:'x-devsquest-ticket-followup-state_id']
          new_state = Ticket::State.find_by(default_create: true)
          if ticket.state_id != new_state.id && !mail[:'x-devsquest-out-of-office']
      ...
      if mail[:'x-devsquest-ticket-tags'].present?
        mail[:'x-devsquest-ticket-tags'].each do |tag|
          ticket.tag_add(tag)
        end
      end
  ...
  def self.check_attributes_by_x_headers(header_name, value)
    ...
    return true if header_name == 'x-devsquest-ticket-tags'

    if header_name =~ %r{^x-devsquest-(.+?)-(followup-|)(.*)$}i
      class_name = $1
      attribute = $3
    end
  ...
  def set_attributes_by_x_headers(item_object, header_name, mail, suffix = false)
    ...
    # loop all x-devsquest-header-* headers
    item_object.attributes.each_key do |key|
      ...
      if key_short == '_id'
        ...
        header = "x-devsquest-#{header_name}-#{key_short}"
        if suffix
          header = "x-devsquest-#{header_name}-#{suffix}-#{key_short}"
        end
      ...
      header = "x-devsquest-#{header_name}-#{key}"
      if suffix
        header = "x-devsquest-#{header_name}-#{suffix}-#{key}"
      end
      if mail[header.to_sym]
...
=end
  ...
  # https://github.com/devsquest/devsquest/issues/2922
  ...
  # https://github.com/devsquest/devsquest/issues/3115
  ...
  def get_attachments(file, attachments, mail)
    ...
    # https://github.com/devsquest/devsquest/issues/928
    if filename.present?
  ...
  def generate_message_id(raw_message, from)
    fqdn = guess_email_fqdn(from) || 'devsquest_generated'

    "<gen-#{Digest::MD5.hexdigest(raw_message)}@#{fqdn}>"
  end

  # https://github.com/devsquest/devsquest/issues/3096
  ...
  #
  # https://github.com/devsquest/devsquest/issues/3368
...
module Mail
  ...
  # https://github.com/devsquest/devsquest/issues/348
  class Body
    def decoded
