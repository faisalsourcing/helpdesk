  ...
  def self.run(_channel, mail, _transaction_params)
    ...
    filters.each do |filter|
      ...
      filter[:perform].each do |key, meta|
        ...
        if key.casecmp('x-devsquest-ticket-tags').zero? && meta['value'].present? && meta['operator'].present?
          mail[ 'x-devsquest-ticket-tags'.downcase.to_sym ] ||= []
          ...
          when 'add'
            tags.each do |tag|
              ...
              tag.strip!
              next if mail[ 'x-devsquest-ticket-tags'.downcase.to_sym ].include?(tag)

              mail[ 'x-devsquest-ticket-tags'.downcase.to_sym ].push tag
            end
          when 'remove'
            tags.each do |tag|
              next if tag.blank?
              ...
              tag.strip!
              mail[ 'x-devsquest-ticket-tags'.downcase.to_sym ] -= [tag]
            end
          end
          next
        end
