...
module Channel::Filter::FollowUpCheck
  ...
  def self.run(_channel, mail, _transaction_params)
    ...
    return true if (setting.include?('references') || (mail[:'x-devsquest-is-auto-response'] == true || Setting.get('ticket_hook_position') == 'none')) && follow_up_by_md5(mail)
    ...
    if mail[:subject].present?
      ...
      if references != ''
        ...
        message_ids.each do |message_id|
          ...
          mail[:'x-devsquest-ticket-id'] = article_first.ticket_id
          return true
        end
      end
    end
  ...
  def self.follow_up_by_md5(mail)
    ...
    mail_references(mail).split(%r{\s+}).each do |message_id|
      ...
      mail[:'x-devsquest-ticket-id'] = article.ticket_id
      return true
    end
  end
end
