...
module Channel::Filter::IdentifySender
  ...
  def self.run(_channel, mail, _transaction_params)
    ...
    customer_user_id = mail[ :'x-devsquest-ticket-customer_id' ]
    customer_user = nil
    if customer_user_id.present?
      customer_user = User.lookup(id: customer_user_id)
      if customer_user
        Rails.logger.debug { "Took customer form x-devsquest-ticket-customer_id header '#{customer_user_id}'." }
      else
        Rails.logger.debug { "Invalid x-devsquest-ticket-customer_id header '#{customer_user_id}', no such user - take user from 'from'-header." }
      end
    end

    # check if sender exists in database
    if !customer_user && mail[ :'x-devsquest-customer-login' ].present?
      customer_user = User.find_by(login: mail[ :'x-devsquest-customer-login' ])
    end
    if !customer_user && mail[ :'x-devsquest-customer-email' ].present?
      customer_user = User.find_by(email: mail[ :'x-devsquest-customer-email' ])
    end

    # get correct customer
    if !customer_user && Setting.get('postmaster_sender_is_agent_search_for_customer') == true && mail[ :'x-devsquest-ticket-create-article-sender' ] == 'Agent'
    ...
    if !customer_user
      customer_user = user_create(
        login:     mail[ :'x-devsquest-customer-login' ] || mail[ :'x-devsquest-customer-email' ] || mail[:from_email],
        firstname: mail[ :'x-devsquest-customer-firstname' ] || mail[:from_display_name],
        lastname:  mail[ :'x-devsquest-customer-lastname' ],
        email:     mail[ :'x-devsquest-customer-email' ] || mail[:from_email],
      )
    end

    create_recipients(mail)
    mail[ :'x-devsquest-ticket-customer_id' ] = customer_user.id
    ...
    if session_user_id.present?
      ...
      if session_user
        Rails.logger.debug { "Took session form x-devsquest-session-user-id header '#{session_user_id}'." }
      else
        Rails.logger.debug { "Invalid x-devsquest-session-user-id header '#{session_user_id}', no such user - take user from 'from'-header." }
      end
    end
    if !session_user
    ...
    if session_user
      mail[ :'x-devsquest-session-user-id' ] = session_user.id
    end
  ...
  def self.sanitize_email(string)
    ...
    string.downcase
          ...
          .delete('"')
          .delete("'")
          .delete(' ') # see https://github.com/devsquest/devsquest/issues/2254
          .sub(%r{^<|>$}, '')        # see https://github.com/devsquest/devsquest/issues/2254
          .sub(%r{\A'(.*)'\z}, '\1') # see https://github.com/devsquest/devsquest/issues/2154
          .gsub(%r{\s}, '')          # see https://github.com/devsquest/devsquest/issues/2198
          .delete_suffix('.')
  end
