...
module Channel::Filter::ImportArchive
  ...
  def self.overwrite_created_at(mail)
    mail[:'x-devsquest-ticket-created_at'] = mail[:date]
    mail[:'x-devsquest-article-created_at'] = mail[:date]
  end

  def self.skip_auto_response(mail)
    mail[:'x-devsquest-is-auto-response'] = true
  end

  def self.ticket_closed(mail)
    closed_state = Ticket::State.by_category(:closed).first
    mail[:'x-devsquest-ticket-state_id'] = closed_state.id
    mail[:'x-devsquest-ticket-followup-state_id'] = closed_state.id
  end
