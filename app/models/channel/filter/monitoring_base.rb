...
class Channel::Filter::MonitoringBase
  ...
  def self.run(_channel, mail, _transaction_params)
    ...
    ticket_ids.each do |ticket_id|
      ...
      mail[ :'x-devsquest-ticket-id' ] = ticket.id
      ...
      if auto_close && result['state'].present? && result['state'].match(%r{#{state_recovery_match}}i)
        ...
        if state
          mail[ :'x-devsquest-ticket-followup-state' ] = state.name
        end
      end
      return true
    end
    ...
    if !mail[ :'x-devsquest-ticket-id' ]
      if !mail[ :'x-devsquest-ticket-preferences' ]
        mail[ :'x-devsquest-ticket-preferences' ] = {}
      end
      preferences = {}
      preferences[integration] = result
      preferences.each do |key, value|
        mail[ :'x-devsquest-ticket-preferences' ][key] = value
      end
    end
