...
class Scheduler < ApplicationModel
  ...
  def self.cleanup(force: false)
    ...
    if !force && caller_locations(1..1).first.label != 'threads'
      raise 'This method should only get called when Scheduler.threads are initialized. Use `force: true` to start anyway.' # rubocop:disable devsquest/DetectTranslatableString
    end
  ...
  def self._start_job(job, try_count = 0, try_run_time = Time.zone.now)
    ...
    if try_run_max > try_count
      # wait between retries (see https://github.com/devsquest/devsquest/issues/1950)
