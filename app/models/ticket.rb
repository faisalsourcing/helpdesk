    ...
    Transaction.execute(local_options) do
      triggers.each do |trigger|
        ...
        if recursive == false && local_options[:loop_count] > 1
          message = "Do not execute recursive triggers per default until devsquest 3.0. With devsquest 3.0 and higher the following trigger is executed '#{trigger.name}' on Ticket:#{ticket.id}. Please review your current triggers and change them if needed."
          logger.info { message }
          return [true, message]
        end
...
=end
  ...
  # (see https://github.com/devsquest/devsquest/issues/1543)
