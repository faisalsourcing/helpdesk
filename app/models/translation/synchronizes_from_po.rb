...
module Translation::SynchronizesFromPo
  ...
  class_methods do # rubocop:disable Metrics/BlockLength
    ...
    # Returns all po files for a locale with devsquest.*.po as first entry,
    #   followed by all other files in alphabetical order
    # For en-us, i18n/devsquest.pot will be returned instead.
    def po_files_for_locale(locale)
      return ['i18n/devsquest.pot'] if locale.eql? 'en-us'

      files = Dir.glob "i18n/*.#{locale}.po", base: Rails.root
      if files.exclude?("i18n/devsquest.#{locale}.po")
        Rails.logger.error "No translation found for locale '#{locale}'."
        return []
      end

      [
        files.delete("i18n/devsquest.#{locale}.po"),
        *files.sort
      ]
    end
  end
