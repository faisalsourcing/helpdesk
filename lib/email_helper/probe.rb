...
=end
    ...
    def self.outbound(params, email, subject = nil)
      ...
      # prepare test email
      # rubocop:disable devsquest/DetectTranslatableString
      mail = if subject
               {
                 from:    email,
                 to:      email,
                 subject: "devsquest Getting started Test Email #{subject}",
                 body:    "This is a test email from devsquest to check if email sending and receiving work correctly.\n\nYou can ignore or delete this email.",
               }
             else
               {
                 from:    email,
                 ...
                 body:    "This is a test email from devsquest to verify if devsquest can send emails to an external address.\n\nIf you see this email, you can ignore or delete it.",
               }
             end
      # rubocop:enable devsquest/DetectTranslatableString

      if subject.present?
        mail['X-devsquest-Test-Message'] = subject
      end
      mail['X-devsquest-Ignore']          = 'true'
      mail['X-devsquest-Fqdn']            = Setting.get('fqdn')
      mail['X-devsquest-Verify']          = 'true'
      mail['X-devsquest-Verify-Time']     = Time.zone.now.iso8601
      mail['X-Loop']                   = 'yes'
