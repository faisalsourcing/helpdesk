...
=end
  ...
  def self.jobs(node_id = nil)
    ...
    # dispatch sessions
    if node_id.blank? && ENV['devsquest_SESSION_JOBS_CONCURRENT'].to_i.positive?
      ...
      node_count     = ENV['devsquest_SESSION_JOBS_CONCURRENT'].to_i
      node_pids      = []
      (1..node_count).each do |worker_node_id|
        node_pids << fork do
          title         = "devsquest Session Jobs Node ##{worker_node_id}: dispatch_pid:#{dispatcher_pid} -> worker_pid:#{Process.pid}"
          $PROGRAM_NAME = title
