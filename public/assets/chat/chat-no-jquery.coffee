do(window) ->
  ...
  class devsquestChat extends Base
    defaults:
      chatId: undefined
      show: true
    ...
    render: =>
      if !@el || !document.querySelector('.devsquest-chat')
        @renderBase()
    ...
    renderBase: ->
      ...
      @el = @options.target.querySelector('.devsquest-chat')
      @input = @el.querySelector('.devsquest-chat-input')
      @body = @el.querySelector('.devsquest-chat-body')
      ...
      @el.querySelector('.devsquest-chat-controls').addEventListener('submit', @onSubmit)
      @body.addEventListener('scroll', @detectScrolledtoBottom)
      @el.querySelector('.devsquest-scroll-hint').addEventListener('click', @onScrollHintClick)
            ...
            switch pipe.data.state
              when 'online'
                ...
                if !@options.cssAutoload || @cssLoaded
                  ...
                  @socketReady = true
              when 'offline'
                @onError 'devsquest Chat: No agent online'
              when 'chat_disabled'
                @onError 'devsquest Chat: Chat is disabled'
              when 'no_seats_available'
                @onError "devsquest Chat: Too many clients in queue. Clients in queue: #{pipe.data.queue}"
              when 'reconnect'
    ...
    onInput: =>
      ...
      for message in @el.querySelectorAll('.devsquest-chat-message--unread')
    ...
    sendMessage: ->
      ...
      if @el.querySelector('.devsquest-chat-message--typing')
        @lastAddedType = 'typing-placeholder'
        @el.querySelector('.devsquest-chat-message--typing').insertAdjacentHTML('beforebegin', messageElement)
      else
    ...
    renderMessage: (data) =>
      ...
      data.unreadClass = if document.hidden then ' devsquest-chat-message--unread' else ''
    ...
    open: =>
      ...
      @el.classList.add 'devsquest-chat-is-open'
      remainerHeight = @el.clientHeight - @el.querySelector('.devsquest-chat-header').offsetHeight
      ...
      if !@sessionId
        ...
        @el.classList.add 'devsquest-chat--animate'
        # force redraw
        @el.clientHeight
        # start animation
        @el.style.transform = ''
    ...
    onOpenAnimationEnd: =>
      ...
      @el.classList.remove 'devsquest-chat--animate'
      @idleTimeout.stop()
    ...
    onCloseAnimationEnd: =>
      ...
      @el.classList.remove 'devsquest-chat-is-open', 'devsquest-chat--animate'
      @el.style.transform = ''

      @showLoader()
      @el.querySelector('.devsquest-chat-welcome').classList.remove('devsquest-chat-is-hidden')
      @el.querySelector('.devsquest-chat-agent').classList.add('devsquest-chat-is-hidden')
      @el.querySelector('.devsquest-chat-agent-status').classList.add('devsquest-chat-is-hidden')
    ...
    onWebSocketClose: =>
      return if @isOpen
      if @el
        @el.classList.remove('devsquest-chat-is-shown')
        @el.classList.remove('devsquest-chat-is-loaded')

    show: ->
      return if @state is 'offline'

      @el.classList.add('devsquest-chat-is-loaded')
      @el.classList.add('devsquest-chat-is-shown')

    disableInput: ->
      @inputDisabled = true
      @input.setAttribute('contenteditable', false)
      @el.querySelector('.devsquest-chat-send').disabled = true
      @io.close()

    enableInput: ->
      @inputDisabled = false
      @input.setAttribute('contenteditable', true)
      @el.querySelector('.devsquest-chat-send').disabled = false

    hideModal: ->
      @el.querySelector('.devsquest-chat-modal').innerHTML = ''
    ...
    onQueue: (data) =>
      ...
      @el.querySelector('.devsquest-chat-modal').innerHTML = @view('waiting')
    ...
    onAgentTypingStart: =>
      ...
      return if @el.querySelector('.devsquest-chat-message--typing')
      ...
      return if !@isVisible(@el.querySelector('.devsquest-chat-message--typing'), true)
      @scrollToBottom()

    onAgentTypingEnd: =>
      @el.querySelector('.devsquest-chat-message--typing').remove() if @el.querySelector('.devsquest-chat-message--typing')
    ...
    updateLastTimestamp: (label, time) ->
      return if !@el
      timestamps = @el.querySelectorAll('.devsquest-chat-body .devsquest-chat-timestamp')
    ...
    detectScrolledtoBottom: =>
      ...
      @el.querySelector('.devsquest-scroll-hint').classList.add('is-hidden') if @scrolledToBottom

    showScrollHint: ->
      @el.querySelector('.devsquest-scroll-hint').classList.remove('is-hidden')
      # compensate scroll
      @body.scrollTop = @body.scrollTop + @el.querySelector('.devsquest-scroll-hint').offsetHeight
    ...
    onConnectionEstablished: (data) =>
      ...
      @el.querySelector('.devsquest-chat-agent').innerHTML = @view('agent')
        agent: @agent

      @enableInput()

      @hideModal()
      @el.querySelector('.devsquest-chat-welcome').classList.add('devsquest-chat-is-hidden')
      @el.querySelector('.devsquest-chat-agent').classList.remove('devsquest-chat-is-hidden')
      @el.querySelector('.devsquest-chat-agent-status').classList.remove('devsquest-chat-is-hidden')
    ...
    showCustomerTimeout: ->
      @el.querySelector('.devsquest-chat-modal').innerHTML = @view('customer_timeout')
        agent: @agent.name
    ...
    showWaitingListTimeout: ->
      @el.querySelector('.devsquest-chat-modal').innerHTML = @view('waiting_list_timeout')
        delay: @options.watingListTimeout
      @el.querySelector('.js-restart').addEventListener 'click', -> location.reload()
      @sessionClose()

    showLoader: ->
      @el.querySelector('.devsquest-chat-modal').innerHTML = @view('loader')()

    setAgentOnlineState: (state) =>
      @state = state
      return if !@el
      capitalizedState = state.charAt(0).toUpperCase() + state.slice(1)
      @el.querySelector('.devsquest-chat-agent-status').dataset.status = state
      @el.querySelector('.devsquest-chat-agent-status').textContent = @T(capitalizedState)
  ...
  window.devsquestChat = devsquestChat
