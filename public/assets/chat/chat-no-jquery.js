...
(function(window) {
  var Base, Core, Io, Log, Timeout, devsquestChat, myScript, scriptHost, scriptProtocol, scripts;
  ...
  devsquestChat = (function(superClass) {
    ...
    function devsquestChat(options) {
      ...
      devsquestChat.__super__.constructor.call(this, options);
    ...
    devsquestChat.prototype.render = function() {
      var btn;
      if (!this.el || !document.querySelector('.devsquest-chat')) {
        this.renderBase();
      }
    ...
    devsquestChat.prototype.renderBase = function() {
      ...
      this.el = this.options.target.querySelector('.devsquest-chat');
      this.input = this.el.querySelector('.devsquest-chat-input');
      this.body = this.el.querySelector('.devsquest-chat-body');
      ...
      this.el.querySelector('.devsquest-chat-controls').addEventListener('submit', this.onSubmit);
      this.body.addEventListener('scroll', this.detectScrolledtoBottom);
      this.el.querySelector('.devsquest-scroll-hint').addEventListener('click', this.onScrollHintClick);
    ...
    devsquestChat.prototype.onDrop = function(e) {
    ...
    devsquestChat.prototype.onPaste = function(e) {
    ...
    devsquestChat.prototype.onKeydown = function(e) {
    ...
    devsquestChat.prototype.onError = function(message) {
      ...
      btn = document.querySelector("." + this.options.buttonClass);
      if (btn) {
        btn.classList.add('devsquest-chat-is-hidden');
      }
      if (this.isOpen) {
        this.disableInput();
        this.destroy({
    ...
    devsquestChat.prototype.onInput = function() {
      ...
      ref = this.el.querySelectorAll('.devsquest-chat-message--unread');
      for (j = 0, len = ref.length; j < len; j++) {
        message = ref[j];
        message.classList.remove('devsquest-chat-message--unread');
      }
    ...
    devsquestChat.prototype.sendMessage = function() {
      ...
      if (this.el.querySelector('.devsquest-chat-message--typing')) {
        this.lastAddedType = 'typing-placeholder';
        this.el.querySelector('.devsquest-chat-message--typing').insertAdjacentHTML('beforebegin', messageElement);
      } else {
    ...
    devsquestChat.prototype.open = function() {
      ...
      this.el.classList.add('devsquest-chat-is-open');
      remainerHeight = this.el.clientHeight - this.el.querySelector('.devsquest-chat-header').offsetHeight;
      this.el.style.transform = "translateY(" + remainerHeight + "px)";
      this.el.clientHeight;
      if (!this.sessionId) {
        this.el.addEventListener('transitionend', this.onOpenAnimationEnd);
        this.el.classList.add('devsquest-chat--animate');
        this.el.clientHeight;
    ...
    devsquestChat.prototype.onOpenAnimationEnd = function() {
      ...
      this.el.classList.remove('devsquest-chat--animate');
    ...
    devsquestChat.prototype.onCloseAnimationEnd = function() {
      ...
      this.el.classList.remove('devsquest-chat-is-open', 'devsquest-chat--animate');
      this.el.style.transform = '';
      this.showLoader();
      this.el.querySelector('.devsquest-chat-welcome').classList.remove('devsquest-chat-is-hidden');
      this.el.querySelector('.devsquest-chat-agent').classList.add('devsquest-chat-is-hidden');
      this.el.querySelector('.devsquest-chat-agent-status').classList.add('devsquest-chat-is-hidden');
      this.isOpen = false;
    ...
    devsquestChat.prototype.onWebSocketClose = function() {
      ...
      if (this.el) {
        this.el.classList.remove('devsquest-chat-is-shown');
        return this.el.classList.remove('devsquest-chat-is-loaded');
      }
    };
    ...
    devsquestChat.prototype.show = function() {
      ...
      this.el.classList.add('devsquest-chat-is-loaded');
      return this.el.classList.add('devsquest-chat-is-shown');
    };

    devsquestChat.prototype.disableInput = function() {
      this.inputDisabled = true;
      this.input.setAttribute('contenteditable', false);
      this.el.querySelector('.devsquest-chat-send').disabled = true;
    ...
    devsquestChat.prototype.enableInput = function() {
      ...
      return this.el.querySelector('.devsquest-chat-send').disabled = false;
    };

    devsquestChat.prototype.hideModal = function() {
      return this.el.querySelector('.devsquest-chat-modal').innerHTML = '';
    };
    ...
    devsquestChat.prototype.onQueue = function(data) {
      ...
      this.inQueue = true;
      return this.el.querySelector('.devsquest-chat-modal').innerHTML = this.view('waiting')({
    ...
    devsquestChat.prototype.onAgentTypingStart = function() {
      ...
      if (!this.isVisible(this.el.querySelector('.devsquest-chat-message--typing'), true)) {
        return;
      }
      return this.scrollToBottom();
    };
    ...
    devsquestChat.prototype.onAgentTypingEnd = function() {
      if (this.el.querySelector('.devsquest-chat-message--typing')) {
        return this.el.querySelector('.devsquest-chat-message--typing').remove();
      }
    };
    ...
    devsquestChat.prototype.updateLastTimestamp = function(label, time) {
      ...
      timestamps = this.el.querySelectorAll('.devsquest-chat-body .devsquest-chat-timestamp');
      if (!timestamps) {
        return;
      }
    ...
    devsquestChat.prototype.detectScrolledtoBottom = function() {
      ...
      if (this.scrolledToBottom) {
        return this.el.querySelector('.devsquest-scroll-hint').classList.add('is-hidden');
      }
    };

    devsquestChat.prototype.showScrollHint = function() {
      this.el.querySelector('.devsquest-scroll-hint').classList.remove('is-hidden');
      return this.body.scrollTop = this.body.scrollTop + this.el.querySelector('.devsquest-scroll-hint').offsetHeight;
    };
    ...
    devsquestChat.prototype.onConnectionEstablished = function(data) {
      ...
      this.body.innerHTML = '';
      this.el.querySelector('.devsquest-chat-agent').innerHTML = this.view('agent')({
        agent: this.agent
      });
      this.enableInput();
      this.hideModal();
      this.el.querySelector('.devsquest-chat-welcome').classList.add('devsquest-chat-is-hidden');
      this.el.querySelector('.devsquest-chat-agent').classList.remove('devsquest-chat-is-hidden');
      this.el.querySelector('.devsquest-chat-agent-status').classList.remove('devsquest-chat-is-hidden');
      if (!this.isFullscreen) {
        this.input.focus();
      }
      this.setAgentOnlineState('online');
    ...
    devsquestChat.prototype.showCustomerTimeout = function() {
      this.el.querySelector('.devsquest-chat-modal').innerHTML = this.view('customer_timeout')({
    ...
    devsquestChat.prototype.showWaitingListTimeout = function() {
      this.el.querySelector('.devsquest-chat-modal').innerHTML = this.view('waiting_list_timeout')({
    ...
    devsquestChat.prototype.showLoader = function() {
      return this.el.querySelector('.devsquest-chat-modal').innerHTML = this.view('loader')();
    };
    ...
    devsquestChat.prototype.setAgentOnlineState = function(state) {
      ...
      this.el.querySelector('.devsquest-chat-agent-status').dataset.status = state;
      return this.el.querySelector('.devsquest-chat-agent-status').textContent = this.T(capitalizedState);
    };
    ...
    devsquestChat.prototype.removeAttributes = function(html) {
      var j, len, node, ref;
