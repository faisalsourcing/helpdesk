do($ = window.jQuery, window) ->
  ...
  class devsquestChat extends Base
    ...
    renderBase: ->
      ...
      @input = @el.find('.devsquest-chat-input')

      # start bindings
      @el.find('.js-chat-open').on 'click', @open
      @el.find('.js-chat-toggle').on 'click', @toggle
      @el.find('.js-chat-status').on 'click', @stopPropagation
      @el.find('.devsquest-chat-controls').on 'submit', @onSubmit
      @el.find('.devsquest-chat-body').on 'scroll', @detectScrolledtoBottom
      @el.find('.devsquest-scroll-hint').on 'click', @onScrollHintClick
      @input.on(
    ...
    onInput: =>
      ...
      @el.find('.devsquest-chat-message--unread')
        .removeClass 'devsquest-chat-message--unread'
    ...
    sendMessage: ->
      ...
      if @el.find('.devsquest-chat-message--typing').get(0)
        @lastAddedType = 'typing-placeholder'
        @el.find('.devsquest-chat-message--typing').before messageElement
      else
        @lastAddedType = 'message--customer'
        @el.find('.devsquest-chat-body').append messageElement
    ...
    renderMessage: (data) =>
      ...
      @el.find('.devsquest-chat-body').append @view('message')(data)
    ...
    open: =>
      ...
      @el.addClass('devsquest-chat-is-open')

      remainerHeight = @el.height() - @el.find('.devsquest-chat-header').outerHeight()
    ...
    onCloseAnimationEnd: =>
      @el.css 'bottom', ''
      @el.removeClass('devsquest-chat-is-open')

      @showLoader()
      @el.find('.devsquest-chat-welcome').removeClass('devsquest-chat-is-hidden')
      @el.find('.devsquest-chat-agent').addClass('devsquest-chat-is-hidden')
      @el.find('.devsquest-chat-agent-status').addClass('devsquest-chat-is-hidden')
    ...
    onWebSocketClose: =>
      return if @isOpen
      if @el
        @el.removeClass('devsquest-chat-is-shown')
        @el.removeClass('devsquest-chat-is-loaded')

    show: ->
      return if @state is 'offline'

      @el.addClass('devsquest-chat-is-loaded')

      @el.addClass('devsquest-chat-is-shown')

    disableInput: ->
      @inputDisabled = true
      @input.prop('contenteditable', false)
      @el.find('.devsquest-chat-send').prop('disabled', true)
      @io.close()

    enableInput: ->
      @inputDisabled = false
      @input.prop('contenteditable', true)
      @el.find('.devsquest-chat-send').prop('disabled', false)

    hideModal: ->
      @el.find('.devsquest-chat-modal').html ''
    ...
    onQueue: (data) =>
      ...
      @el.find('.devsquest-chat-modal').html @view('waiting')
    ...
    onAgentTypingStart: =>
      ...
      return if @el.find('.devsquest-chat-message--typing').get(0)

      @maybeAddTimestamp()

      @el.find('.devsquest-chat-body').append @view('typingIndicator')()

      # only if typing indicator is shown
      return if !@isVisible(@el.find('.devsquest-chat-message--typing'), true)
      @scrollToBottom()

    onAgentTypingEnd: =>
      @el.find('.devsquest-chat-message--typing').remove()
    ...
    maybeAddTimestamp: ->
      ...
      if !@lastTimestamp or (timestamp - @lastTimestamp) > @showTimeEveryXMinutes * 60000
        ...
        if @lastAddedType is 'timestamp'
          ...
          # add new timestamp
          @el.find('.devsquest-chat-body').append @view('timestamp')
            label: label
            time: time
          @lastTimestamp = timestamp
    ...
    updateLastTimestamp: (label, time) ->
      return if !@el
      @el.find('.devsquest-chat-body')
        .find('.devsquest-chat-timestamp')
        .last()
    ...
    addStatus: (status) ->
      ...
      @el.find('.devsquest-chat-body').append @view('status')
        status: status

      @scrollToBottom()

    detectScrolledtoBottom: =>
      scrollBottom = @el.find('.devsquest-chat-body').scrollTop() + @el.find('.devsquest-chat-body').outerHeight()
      @scrolledToBottom = Math.abs(scrollBottom - @el.find('.devsquest-chat-body').prop('scrollHeight')) <= @scrollSnapTolerance
      @el.find('.devsquest-scroll-hint').addClass('is-hidden') if @scrolledToBottom

    showScrollHint: ->
      @el.find('.devsquest-scroll-hint').removeClass('is-hidden')
      # compensate scroll
      @el.find('.devsquest-chat-body').scrollTop(@el.find('.devsquest-chat-body').scrollTop() + @el.find('.devsquest-scroll-hint').outerHeight())

    onScrollHintClick: =>
      # animate scroll
      @el.find('.devsquest-chat-body').animate({scrollTop: @el.find('.devsquest-chat-body').prop('scrollHeight')}, 300)

    scrollToBottom: ({ showHint } = { showHint: false }) ->
      if @scrolledToBottom
        @el.find('.devsquest-chat-body').scrollTop($('.devsquest-chat-body').prop('scrollHeight'))
      else if showHint
        @showScrollHint()
    ...
    onConnectionEstablished: (data) =>
      ...
      # empty old messages
      @el.find('.devsquest-chat-body').html('')

      @el.find('.devsquest-chat-agent').html @view('agent')
        agent: @agent

      @enableInput()

      @hideModal()
      @el.find('.devsquest-chat-welcome').addClass('devsquest-chat-is-hidden')
      @el.find('.devsquest-chat-agent').removeClass('devsquest-chat-is-hidden')
      @el.find('.devsquest-chat-agent-status').removeClass('devsquest-chat-is-hidden')
    ...
    showCustomerTimeout: ->
      @el.find('.devsquest-chat-modal').html @view('customer_timeout')
        agent: @agent.name
    ...
    showWaitingListTimeout: ->
      @el.find('.devsquest-chat-modal').html @view('waiting_list_timeout')
    ...
    showLoader: ->
      @el.find('.devsquest-chat-modal').html @view('loader')()
    ...
    setAgentOnlineState: (state) =>
      ...
      @el
        .find('.devsquest-chat-agent-status')
