...
(function($, window) {
  var Base, Io, Log, Timeout, devsquestChat, myScript, scriptHost, scriptProtocol, scripts;
  ...
  devsquestChat = (function(superClass) {
    extend(devsquestChat, superClass);

    devsquestChat.prototype.defaults = {
      chatId: void 0,
      show: true,
      target: $('body'),
      host: '',
      ...
      cssUrl: void 0,
      fontSize: void 0,
      buttonClass: 'open-devsquest-chat',
    ...
    devsquestChat.prototype.logPrefix = 'chat';

    devsquestChat.prototype._messageCount = 0;

    devsquestChat.prototype.isOpen = false;

    devsquestChat.prototype.blinkOnlineInterval = null;

    devsquestChat.prototype.stopBlinOnlineStateTimeout = null;

    devsquestChat.prototype.showTimeEveryXMinutes = 2;

    devsquestChat.prototype.lastTimestamp = null;

    devsquestChat.prototype.lastAddedType = null;

    devsquestChat.prototype.inputDisabled = false;

    devsquestChat.prototype.inputTimeout = null;

    devsquestChat.prototype.isTyping = false;

    devsquestChat.prototype.state = 'offline';

    devsquestChat.prototype.initialQueueDelay = 10000;

    devsquestChat.prototype.translations = {
      'da': {
    ...
    devsquestChat.prototype.sessionId = void 0;

    devsquestChat.prototype.scrolledToBottom = true;

    devsquestChat.prototype.scrollSnapTolerance = 10;

    devsquestChat.prototype.richTextFormatKey = {
      66: true,
      73: true,
      85: true,
      83: true
    ...
    devsquestChat.prototype.T = function() {
    ...
    devsquestChat.prototype.view = function(name) {
      return (function(_this) {
        return function(options) {
          ...
          return window.devsquestChatTemplates[name](options);
        };
      })(this);
    };

    function devsquestChat(options) {
      ...
      this.options = $.extend({}, this.defaults, options);
      devsquestChat.__super__.constructor.call(this, this.options);
    ...
    devsquestChat.prototype.getScrollRoot = function() {
      var end, html, start;
    ...
    devsquestChat.prototype.render = function() {
      if (!this.el || !$('.devsquest-chat').get(0)) {
        this.renderBase();
      }
    ...
    devsquestChat.prototype.renderBase = function() {
      this.el = $(this.view('chat')({
        title: this.options.title,
        scrollHint: this.options.scrollHint
      }));
      this.options.target.append(this.el);
      this.input = this.el.find('.devsquest-chat-input');
      ...
      this.el.find('.devsquest-chat-controls').on('submit', this.onSubmit);
      this.el.find('.devsquest-chat-body').on('scroll', this.detectScrolledtoBottom);
      this.el.find('.devsquest-scroll-hint').on('click', this.onScrollHintClick);
      this.input.on({
    ...
    devsquestChat.prototype.stopPropagation = function(event) {
      return event.stopPropagation();
    };

    devsquestChat.prototype.checkForEnter = function(event) {
    ...
    devsquestChat.prototype.send = function(event, data) {
      if (data == null) {
        data = {};
      }
    ...
    devsquestChat.prototype.onWebSocketMessage = function(pipes) {
      var j, len, pipe;
      for (j = 0, len = pipes.length; j < len; j++) {
        pipe = pipes[j];
          ...
          case 'chat_status_customer':
            switch (pipe.data.state) {
              case 'online':
                ...
                break;
              case 'offline':
                this.onError('devsquest Chat: No agent online');
                break;
              case 'chat_disabled':
                this.onError('devsquest Chat: Chat is disabled');
                break;
              case 'no_seats_available':
                this.onError("devsquest Chat: Too many clients in queue. Clients in queue: " + pipe.data.queue);
                break;
              case 'reconnect':
    ...
    devsquestChat.prototype.onReady = function() {
      var base;
    ...
    devsquestChat.prototype.onError = function(message) {
      var base;
    ...
    devsquestChat.prototype.onReopenSession = function(data) {
    ...
    devsquestChat.prototype.onInput = function() {
      this.el.find('.devsquest-chat-message--unread').removeClass('devsquest-chat-message--unread');
      sessionStorage.setItem('unfinished_message', this.input.html());
      return this.onTyping();
    };

    devsquestChat.prototype.onFocus = function() {
      var keyboardShown;
      $(window).scrollTop(10);
    ...
    devsquestChat.prototype.onFocusOut = function() {};

    devsquestChat.prototype.onTyping = function() {
    ...
    devsquestChat.prototype.onSubmit = function(event) {
      event.preventDefault();
      return this.sendMessage();
    };

    devsquestChat.prototype.sendMessage = function() {
      ...
      if (this.el.find('.devsquest-chat-message--typing').get(0)) {
        this.lastAddedType = 'typing-placeholder';
        this.el.find('.devsquest-chat-message--typing').before(messageElement);
      } else {
        this.lastAddedType = 'message--customer';
        this.el.find('.devsquest-chat-body').append(messageElement);
      }
      this.input.html('');
      this.scrollToBottom();
    ...
    devsquestChat.prototype.receiveMessage = function(data) {
      this.inactiveTimeout.start();
    ...
    devsquestChat.prototype.renderMessage = function(data) {
      this.lastAddedType = "message--" + data.from;
      data.unreadClass = document.hidden ? ' devsquest-chat-message--unread' : '';
      return this.el.find('.devsquest-chat-body').append(this.view('message')(data));
    };

    devsquestChat.prototype.open = function() {
      var remainerHeight;
      if (this.isOpen) {
      ...
      this.el.addClass('devsquest-chat-is-open');
      remainerHeight = this.el.height() - this.el.find('.devsquest-chat-header').outerHeight();
    ...
    devsquestChat.prototype.onOpenAnimationEnd = function() {
      var base;
    ...
    devsquestChat.prototype.sessionClose = function() {
    ...
    devsquestChat.prototype.toggle = function(event) {
      if (this.isOpen) {
    ...
    devsquestChat.prototype.close = function(event) {
      var remainerHeight;
      if (!this.isOpen) {
    ...
    devsquestChat.prototype.onCloseAnimationEnd = function() {
      var base;
      this.el.css('bottom', '');
      this.el.removeClass('devsquest-chat-is-open');
      this.showLoader();
      this.el.find('.devsquest-chat-welcome').removeClass('devsquest-chat-is-hidden');
      this.el.find('.devsquest-chat-agent').addClass('devsquest-chat-is-hidden');
      this.el.find('.devsquest-chat-agent-status').addClass('devsquest-chat-is-hidden');
      this.isOpen = false;
    ...
    devsquestChat.prototype.onWebSocketClose = function() {
      if (this.isOpen) {
        return;
      }
      if (this.el) {
        this.el.removeClass('devsquest-chat-is-shown');
        return this.el.removeClass('devsquest-chat-is-loaded');
      }
    };

    devsquestChat.prototype.show = function() {
      if (this.state === 'offline') {
        return;
      }
      this.el.addClass('devsquest-chat-is-loaded');
      return this.el.addClass('devsquest-chat-is-shown');
    };

    devsquestChat.prototype.disableInput = function() {
      this.inputDisabled = true;
      this.input.prop('contenteditable', false);
      this.el.find('.devsquest-chat-send').prop('disabled', true);
      return this.io.close();
    };

    devsquestChat.prototype.enableInput = function() {
      this.inputDisabled = false;
      this.input.prop('contenteditable', true);
      return this.el.find('.devsquest-chat-send').prop('disabled', false);
    };

    devsquestChat.prototype.hideModal = function() {
      return this.el.find('.devsquest-chat-modal').html('');
    };

    devsquestChat.prototype.onQueueScreen = function(data) {
      var show;
    ...
    devsquestChat.prototype.onQueue = function(data) {
      this.log.notice('onQueue', data.position);
      this.inQueue = true;
      return this.el.find('.devsquest-chat-modal').html(this.view('waiting')({
        position: data.position
      }));
    };

    devsquestChat.prototype.onAgentTypingStart = function() {
      ...
      this.maybeAddTimestamp();
      this.el.find('.devsquest-chat-body').append(this.view('typingIndicator')());
      if (!this.isVisible(this.el.find('.devsquest-chat-message--typing'), true)) {
        return;
      }
      return this.scrollToBottom();
    };

    devsquestChat.prototype.onAgentTypingEnd = function() {
      return this.el.find('.devsquest-chat-message--typing').remove();
    };

    devsquestChat.prototype.onLeaveTemporary = function() {
      if (!this.sessionId) {
        return;
      }
    ...
    devsquestChat.prototype.maybeAddTimestamp = function() {
      ...
      if (!this.lastTimestamp || (timestamp - this.lastTimestamp) > this.showTimeEveryXMinutes * 60000) {
        ...
        if (this.lastAddedType === 'timestamp') {
          ...
          this.el.find('.devsquest-chat-body').append(this.view('timestamp')({
            label: label,
            time: time
          }));
    ...
    devsquestChat.prototype.updateLastTimestamp = function(label, time) {
      if (!this.el) {
        return;
      }
      return this.el.find('.devsquest-chat-body').find('.devsquest-chat-timestamp').last().replaceWith(this.view('timestamp')({
        label: label,
        time: time
      }));
    };

    devsquestChat.prototype.addStatus = function(status) {
      if (!this.el) {
        return;
      }
      this.maybeAddTimestamp();
      this.el.find('.devsquest-chat-body').append(this.view('status')({
        status: status
      }));
      return this.scrollToBottom();
    };

    devsquestChat.prototype.detectScrolledtoBottom = function() {
      var scrollBottom;
      scrollBottom = this.el.find('.devsquest-chat-body').scrollTop() + this.el.find('.devsquest-chat-body').outerHeight();
      this.scrolledToBottom = Math.abs(scrollBottom - this.el.find('.devsquest-chat-body').prop('scrollHeight')) <= this.scrollSnapTolerance;
      if (this.scrolledToBottom) {
        return this.el.find('.devsquest-scroll-hint').addClass('is-hidden');
      }
    };

    devsquestChat.prototype.showScrollHint = function() {
      this.el.find('.devsquest-scroll-hint').removeClass('is-hidden');
      return this.el.find('.devsquest-chat-body').scrollTop(this.el.find('.devsquest-chat-body').scrollTop() + this.el.find('.devsquest-scroll-hint').outerHeight());
    };

    devsquestChat.prototype.onScrollHintClick = function() {
      return this.el.find('.devsquest-chat-body').animate({
        scrollTop: this.el.find('.devsquest-chat-body').prop('scrollHeight')
      }, 300);
    };

    devsquestChat.prototype.scrollToBottom = function(arg) {
      var showHint;
      showHint = (arg != null ? arg : {
        showHint: false
      }).showHint;
      if (this.scrolledToBottom) {
        return this.el.find('.devsquest-chat-body').scrollTop($('.devsquest-chat-body').prop('scrollHeight'));
      } else if (showHint) {
        return this.showScrollHint();
      }
    };

    devsquestChat.prototype.destroy = function(params) {
      if (params == null) {
        params = {};
      }
    ...
    devsquestChat.prototype.reconnect = function() {
    ...
    devsquestChat.prototype.onConnectionReestablished = function() {
      var base;
      this.lastAddedType = 'status';
      this.setAgentOnlineState('online');
    ...
    devsquestChat.prototype.onSessionClosed = function(data) {
      var base;
    ...
    devsquestChat.prototype.setSessionId = function(id) {
      this.sessionId = id;
      if (id === void 0) {
    ...
    devsquestChat.prototype.onConnectionEstablished = function(data) {
      var base;
      if (this.onInitialQueueDelayId) {
        clearTimeout(this.onInitialQueueDelayId);
      }
      ...
      this.el.find('.devsquest-chat-body').html('');
      this.el.find('.devsquest-chat-agent').html(this.view('agent')({
        agent: this.agent
      }));
      this.enableInput();
      this.hideModal();
      this.el.find('.devsquest-chat-welcome').addClass('devsquest-chat-is-hidden');
      this.el.find('.devsquest-chat-agent').removeClass('devsquest-chat-is-hidden');
      this.el.find('.devsquest-chat-agent-status').removeClass('devsquest-chat-is-hidden');
      if (!this.isFullscreen) {
    ...
    devsquestChat.prototype.showCustomerTimeout = function() {
      var reload;
      this.el.find('.devsquest-chat-modal').html(this.view('customer_timeout')({
    ...
    devsquestChat.prototype.showWaitingListTimeout = function() {
      var reload;
      this.el.find('.devsquest-chat-modal').html(this.view('waiting_list_timeout')({
    ...
    devsquestChat.prototype.showLoader = function() {
      return this.el.find('.devsquest-chat-modal').html(this.view('loader')());
    };

    devsquestChat.prototype.setAgentOnlineState = function(state) {
      ...
      return this.el.find('.devsquest-chat-agent-status').attr('data-status', state).text(this.T(capitalizedState));
    };

    devsquestChat.prototype.detectHost = function() {
      var protocol;
      protocol = 'ws://';
    ...
    devsquestChat.prototype.loadCss = function() {
    ...
    devsquestChat.prototype.onCssLoaded = function() {
      var base;
      this.cssLoaded = true;
      if (this.socketReady) {
        this.onReady();
    ...
    devsquestChat.prototype.startTimeoutObservers = function() {
    ...
    devsquestChat.prototype.disableScrollOnRoot = function() {
    ...
    devsquestChat.prototype.enableScrollOnRoot = function() {
    ...
    devsquestChat.prototype.isVisible = function(el, partial, hidden, direction) {
    ...
    devsquestChat.prototype.isRetina = function() {
      var mq;
    ...
    devsquestChat.prototype.resizeImage = function(dataURL, x, y, sizeFactor, type, quallity, callback, force) {
      var imageObject;
      if (x == null) {
        x = 'auto';
      }
    ...
    devsquestChat.prototype.pasteHtmlAtCaret = function(html) {
    ...
    devsquestChat.prototype.wordFilter = function(editor) {
    ...
    devsquestChat.prototype.removeAttribute = function(element) {
    ...
    devsquestChat.prototype.removeAttributes = function(html, parent) {
      if (parent == null) {
        parent = true;
      }
      if (parent) {
    ...
    return devsquestChat;

  })(Base);
  return window.devsquestChat = devsquestChat;
})(window.jQuery, window);

if (!window.devsquestChatTemplates) {
  window.devsquestChatTemplates = {};
}
window.devsquestChatTemplates["agent"] = function (__obj) {
  if (!__obj) __obj = {};
  ...
  (function() {
    (function() {
      if (this.agent.avatar) {
        __out.push('\n<img class="devsquest-chat-agent-avatar" src="');
        __out.push(__sanitize(this.agent.avatar));
        __out.push('">\n');
      }
    
      __out.push('\n<span class="devsquest-chat-agent-sentence">\n  <span class="devsquest-chat-agent-name">');
...
window.devsquestChatTemplates["chat"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<div class="devsquest-chat');
    
      if (this.flat) {
        __out.push(__sanitize(' devsquest-chat--flat'));
      }
...
window.devsquestChatTemplates["customer_timeout"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<div class="devsquest-chat-modal-text">\n  ');
      ...
      __out.push('\n  <br>\n  <div class="devsquest-chat-button js-restart"');
...
window.devsquestChatTemplates["loader"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<span class="devsquest-chat-loading-animation">\n  <span class="devsquest-chat-loading-circle"></span>\n  <span class="devsquest-chat-loading-circle"></span>\n  <span class="devsquest-chat-loading-circle"></span>\n</span>\n<span class="devsquest-chat-modal-text">');
...
window.devsquestChatTemplates["message"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<div class="devsquest-chat-message devsquest-chat-message--');
    
      __out.push(__sanitize(this.from));
    
      __out.push(__sanitize(this.unreadClass));
    
      __out.push('">\n  <span class="devsquest-chat-message-body"');
...
window.devsquestChatTemplates["status"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<div class="devsquest-chat-status">\n  <div class="devsquest-chat-status-inner">\n    ');
...
window.devsquestChatTemplates["timestamp"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<div class="devsquest-chat-timestamp"><strong>');
...
window.devsquestChatTemplates["typingIndicator"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
  ...
  (function() {
    (function() {
      __out.push('<div class="devsquest-chat-message devsquest-chat-message--typing devsquest-chat-message--agent">\n  <span class="devsquest-chat-message-body">\n    <span class="devsquest-chat-loading-animation">\n      <span class="devsquest-chat-loading-circle"></span>\n      <span class="devsquest-chat-loading-circle"></span>\n      <span class="devsquest-chat-loading-circle"></span>\n    </span>\n  </span>\n</div>');
...
window.devsquestChatTemplates["waiting"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
...
window.devsquestChatTemplates["waiting_list_timeout"] = function (__obj) {
  if (!__obj) __obj = {};
  var __out = [], __capture = function(callback) {
    var out = __out, result;
    __out = [];
