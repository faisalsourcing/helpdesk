...
/*
*
*  provides feedback form for devsquest
*

<button id="devsquest-feedback-form">Feedback</button>

<script id="devsquest_form_script" src="http://localhost:3000/assets/form/form.js"></script>
<script>
$(function() {
  $('#devsquest-feedback-form').devsquestForm({
...
*/
  ...
  var pluginName = 'devsquestForm',
  defaults = {
    lang: undefined,
    debug: false,
    noCSS: false,
    prefixCSS: 'devsquest-form-',
    showTitle: false,
    messageTitle: 'devsquest Form',
  ...
  function Plugin(element, options) {
    ...
    this._src = document.getElementById('devsquest_form_script').src
  ...
  Plugin.prototype.init = function () {
    ...
    // bind form on call
      ...
      this.$element.off('click.devsquest-form').on('click.devsquest-form', function (e) {
        e.preventDefault()
        _this.render()
        return true
      })
  ...
  Plugin.prototype.submit = function() {
    ...
    $.ajax({
      ...
      _this.$form.find('.devsquest-form-group--has-error').removeClass('devsquest-form-group--has-error')
      ...
      if (data.errors) {
        $.each(data.errors, function( key, value ) {
          _this.$form.find('[name=' + key + ']').closest('.'+ _this.options.prefixCSS +'group').addClass('devsquest-form-group--has-error')
          // Deprecated code, can be removed in future versions:
  ...
  Plugin.prototype.render = function(e) {
    ...
    var element = "<div class=\"" + _this.options.prefixCSS + "modal\">\
      <div class=\"" + _this.options.prefixCSS + "modal-backdrop js-devsquest-form-modal-backdrop\"></div>\
      <div class=\"" + _this.options.prefixCSS + "modal-body js-devsquest-form-modal-body\">\
        <form class=\"devsquest-form\"></form>\
      </div>\
    </div>"

    if (!this.options.modal) {
      element = '<div><form class="devsquest-form"></form></div>'
    }
    ...
    // bind on close
    $element.find('.js-devsquest-form-modal-backdrop').off('click.devsquest-form').on('click.devsquest-form', function (e) {
      e.preventDefault()
      _this.closeModal()
      return true
    })
    ...
    // bind form submit
    $element.off('submit.devsquest-form').on('submit.devsquest-form', function (e) {
      e.preventDefault()
      _this.submit()
      return true
    })
  ...
  Plugin.prototype.thanks = function(data) {
    ...
    var message = $('<div class="js-thankyou devsquest-form-thankyou">' + thankYou + '</div>')
