  ...
  describe 'core behavior', :use_vcr do
    ...
    context 'for tweets' do
      let(:tweet_attributes) do
        {
          ...
          'place'               => {},
          'retweet_count'       => 0,
          'source'              => '<a href="https://devsquest.com" rel="nofollow">devsquest Integration Test</a>',
          'favorited'           => false,
          'truncated'           => false,
        }
      end
      ...
      context 'with a user mention' do
        let(:factory_options) { { body: '@devsquesttesting Don’t mind me, just testing the API' } }

        it 'updates the article with tweet recipients' do
          expect { described_class.perform_now(article.id) }
            .to change { article.reload.to }.to('@devsquestTesting')
        end
      end
    end
