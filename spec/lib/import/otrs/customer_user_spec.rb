...
RSpec.describe Import::OTRS::CustomerUser do
  ...
  def creates_with(devsquest_structure)
    allow_organization_lookup
    allow(import_object).to receive(:new).with(devsquest_structure).and_call_original
  ...
  def updates_with(devsquest_structure)
    allow_organization_lookup
    allow(import_object).to receive(:find_by).and_return(existing_object)
    # we delete the :role_ids from the devsquest_structure to make sure that
    # a) role_ids call returns the initial role_ids
    # b) and update! gets called without them
    allow(existing_object).to receive(:role_ids).and_return(devsquest_structure.delete(:role_ids)).at_least(:once)

    expect(existing_object).to receive(:update!).with(devsquest_structure)
  ...
  context 'regular user' do
    ...
    it 'creates' do
      creates_with(devsquest_structure)
    end

    it 'updates' do
      updates_with(devsquest_structure)
    end
  end
