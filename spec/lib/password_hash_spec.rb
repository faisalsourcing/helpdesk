...
RSpec.describe PasswordHash do
  ...
  context 'legacy' do
    ...
    let(:devsquest_sha2) { '{sha2}dd9c764fa7ea18cd992c8600006d3dc3ac983d1ba22e9ba2d71f6207456be0ba' }
    ...
    it 'requires password to be not nil' do
      expect(described_class).not_to be_legacy(devsquest_sha2, nil)
    end

    it 'detects sha2 hashes' do
      expect(described_class.legacy?(devsquest_sha2, pw_plain)).to be true
    end

    it 'detects crypted passwords' do
      expect(described_class.crypted?(devsquest_sha2)).to be true
    end
    ...
    describe '::sha2' do
      ...
      it 'creates sha2 hashes' do
        ...
        expect(hashed).to eq devsquest_sha2
      end
    end
    ...
    describe '::hashed_sha2?' do
      ...
      it 'detects sha2 hashes' do
        expect(described_class.hashed_sha2?(devsquest_sha2)).to be true
      end
    end
  end
