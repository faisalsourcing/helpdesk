      ...
      context 'private key present' do
        ...
        context 'when message is 7bit or 8bit encoded' do
          ...
          context 'when Content-Type is text/plain' do
            ...
            it 'verifies' do
              expect(mail['x-devsquest-article-preferences']['security']['sign']['success']).to be true
            end
          end
    ...
    context 'signature verification' do
      ...
      context 'sender certificate present' do
        ...
        it 'verifies' do
          ...
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:success]).to be true
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to eq(sender_certificate_subject)
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be false
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to be_nil
        end
        ...
        context 'expired' do
          ...
          it 'verifies with comment' do
            expect(mail[:body]).to include(raw_body)
            expect(mail['x-devsquest-article-preferences'][:security][:sign][:success]).to be true
            expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to include(expired_email_address).and include('expired')
            expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be false
            expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to be_nil
          end
      ...
      context 'no sender certificate' do
        ...
        it 'fails' do
          expect(mail[:body]).to include(raw_body)
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:success]).to be false
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to eq('Certificate for verification could not be found.')
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be false
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to be_nil
        end
        ...
        context 'root CA present' do
          ...
          it 'verifies' do
            expect(mail[:body]).to include(raw_body)
            expect(mail['x-devsquest-article-preferences'][:security][:sign][:success]).to be true
            expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to eq(ca_certificate_subject)
            expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be false
            expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to be_nil
          end
        ...
        context 'certificate chain' do
          ...
          context 'certificate chain only partly present' do
            ...
            it 'verifies' do
              expect(mail[:body]).to include(raw_body)
              expect(mail['x-devsquest-article-preferences'][:security][:sign][:success]).to be true
              expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to eq(ca_subject_chain)
              expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be false
              expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to be_nil
            end
          end
    ...
    context 'decryption' do
      ...
      context 'private key present' do
        ...
        it 'decrypts' do
          expect(mail[:body]).to include(raw_body)
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:success]).to be false
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to be_nil
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be true
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to eq(recipient_certificate_subject)
        end
        ...
        context 'expired allowed' do
          ...
          it 'decrypts with comment' do
            ...
            expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to be_nil
            expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be true
            expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to include(expired_email_address).and include('expired')
          end
      ...
      context 'no private key present' do
        ...
        it 'fails' do
          ...
          expect(mail['x-devsquest-article-preferences'][:security][:sign][:comment]).to be_nil
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:success]).to be false
          expect(mail['x-devsquest-article-preferences'][:security][:encryption][:comment]).to eq('Private key for decryption could not be found.')
        end
