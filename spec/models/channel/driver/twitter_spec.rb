...
RSpec.describe Channel::Driver::Twitter, required_envs: %w[TWITTER_CONSUMER_KEY TWITTER_CONSUMER_SECRET TWITTER_OAUTH_TOKEN TWITTER_OAUTH_TOKEN_SECRET TWITTER_DM_RECIPIENT TWITTER_USER_ID] do
  ...
  describe '#process', current_user_id: 1 do
    ...
    # https://git.devsquest.com/devsquest/devsquest/-/issues/305
      ...
      describe 'article creation' do
        ...
        context 'when message contains shortened (t.co) url' do
          ...
          it 'replaces the t.co url for the original' do
            expect { channel.process(payload) }
              .to change { Ticket::Article.exists?(body: <<~BODY.chomp) }.to(true)
                @ScruffyMcG https://devsquest.org/
              BODY
          end
        end
      ...
      context 'with search term configured (at .options[:sync][:search])' do
        it 'creates an article for each recent tweet' do
          ...
          expect(Ticket.last.attributes).to include(
            'title'       => "Come and join our team to bring devsquest even further forward!   It's gonna be ama...",
        ...
        context 'for responses to other tweets' do
          ...
          it 'creates articles for parent tweets as well' do
            ...
            expect(thread.articles.last.body).to match(%r{devsquest}i)       # search result
            expect(thread.articles.first.body).not_to match(%r{devsquest}i)  # parent tweet
          end
        end
          ...
          describe 'Race condition: when #fetch finds a half-processed, outgoing tweet' do
            subject!(:channel) do
              create(:twitter_channel,
                     search_term:    'devsquestdevsquestdevsquest',
                     custom_options: {
                       user: {
            ...
            let!(:tweet) { create(:twitter_article, body: 'devsquestdevsquestdevsquest') }
