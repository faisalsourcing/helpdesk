...
RSpec.describe Channel::EmailParser, type: :model do
    ...
    describe 'handling HTML links in message content' do
      context 'with under 5,000 links' do
        it 'parses message content as normal' do
          expect(described_class.new.parse(<<~RAW)[:body]).to start_with('<a href="https://devsquest.com/"')
            From: nicole.braun@devsquest.com
            Content-Type: text/html

            <html><body>
            #{Array.new(10) { '<a href="https://devsquest.com/">Dummy Link</a>' }.join(' ')}
            </body></html>
          RAW
        end
      end
      ...
      context 'with 5,000+ links' do
        it 'replaces message content with error message' do
          expect(described_class.new.parse(<<~RAW)).to include('body' => Channel::EmailParser::EXCESSIVE_LINKS_MSG)
            ...
            <html><body>
            #{Array.new(5001) { '<a href="https://devsquest.com/">Dummy Link</a>' }.join(' ')}
            </body></html>
          RAW
        end
      end
  ...
  describe '#process' do
      ...
      context 'with a large number of unrecognized recipient addresses' do
        it 'never creates more than 40 users' do
          expect { described_class.new.process({}, <<~RAW) }.to change(User, :count).by(40)
            From: nicole.braun@devsquest.org
    ...
    describe 'creating new tickets' do
      ...
      context 'when email contains x-headers' do
        let(:raw_mail) { <<~RAW.chomp }
          ...
          To: baz@qux.net
          Subject: Foo
          X-devsquest-Ticket-priority: 3 high
    ...
    describe 'associating emails to existing tickets' do
      ...
      describe 'based on where a ticket reference appears in the message' do
        ...
        shared_context 'ticket reference in In-Reply-To header' do
          ...
          let!(:article) { create(:ticket_article, ticket: ticket, message_id: '<20150830145601.30.608882@edenhofer.devsquest.com>') }
        end
    ...
    describe 'formatting to/from addresses' do
      # see https://github.com/devsquest/devsquest/issues/2198
      ...
      # see https://github.com/devsquest/devsquest/issues/2254
    ...
    describe 'charset handling' do
      # see https://github.com/devsquest/devsquest/issues/2224
    ...
    describe 'attachment handling' do
      context 'with header "Content-Transfer-Encoding: x-uuencode"' do
        ...
        it 'parses the content correctly' do
          ...
          expect(article.attachments.first.content).to eq('Hello devsquest')
        end
      end

      # https://github.com/devsquest/devsquest/issues/3529
      context 'Attachments sent by devsquest not shown in Outlook' do
        subject(:mail) do
    ...
    describe 'inline image handling' do
      # see https://github.com/devsquest/devsquest/issues/2486
    ...
    context 'for “delivery failed” notifications (a.k.a. bounce messages)' do
      ...
      context 'with future retries (delayed)' do
        ...
        context 'on a closed ticket' do
          ...
          it 'returns a Mail object with an x-devsquest-out-of-office header' do
            output_mail = described_class.new.process({}, raw_mail).last
            expect(output_mail).to include('x-devsquest-out-of-office': true)
          end
    ...
    context 'for “out-of-office” notifications (a.k.a. auto-response messages)' do
      ...
      context 'on an existing, closed ticket' do
        ...
        context 'when OutOfOfficeCheck filter applies x-devsquest-out-of-office: false' do
          before do
            allow(Channel::Filter::OutOfOfficeCheck)
              .to receive(:run) { |_, mail_hash| mail_hash[:'x-devsquest-out-of-office'] = false }
          end
        ...
        context 'when OutOfOfficeCheck filter applies x-devsquest-out-of-office: true' do
          before do
            allow(Channel::Filter::OutOfOfficeCheck)
              .to receive(:run) { |_, mail_hash| mail_hash[:'x-devsquest-out-of-office'] = true }
          end
      ...
      include_examples 'postmaster reply' do
        ...
        let(:expected_body) do
          body = <<~BODY
            ...
            Postmaster of devsquest.example.com
          BODY
          body.gsub(%r{\n}, "\r\n")
        end
      end
      ...
      include_examples 'postmaster reply' do
        ...
        let(:expected_body) do
          body = <<~BODY
            ...
            Postmaster von devsquest.example.com
          BODY
          body.gsub(%r{\n}, "\r\n")
        end
      end
