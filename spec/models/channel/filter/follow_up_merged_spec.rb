...
RSpec.describe Channel::Filter::FollowUpMerged, type: :channel_filter do
  ...
  context 'merged' do
    ...
    it 'handles ticket merged to one ticket' do
      ...
      mail = {
        'x-devsquest-ticket-id': ticket1.id
      }

      filter(mail)

      expect(mail[:'x-devsquest-ticket-id']).to eq(ticket2.id)
    end
    ...
    it 'handles first merged chain' do
      ...
      expect(mail[:'x-devsquest-ticket-id']).to eq(ticket3.id)
    end
    ...
    it 'handles ticket in merged ticket chain' do
      ...
      mail = {
        'x-devsquest-ticket-id': ticket2.id
      }

      filter(mail)

      expect(mail[:'x-devsquest-ticket-id']).to eq(ticket4.id)
    end
  end
  ...
  context 'ignored mails' do
    ...
    it 'ignores not existing tickets' do
      mail = {
        'x-devsquest-ticket-id': 1_234_567_890
      }
