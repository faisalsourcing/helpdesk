...
RSpec.describe Channel::Filter::OutOfOfficeCheck, type: :channel_filter do
  describe '.run' do
    ...
    shared_examples 'regular message' do
      it 'sets x-devsquest-out-of-office header to false' do
        expect { filter(mail_hash) }
          .to change { mail_hash[:'x-devsquest-out-of-office'] }.to(false)
      end
    end

    shared_examples 'auto-response' do
      it 'sets x-devsquest-out-of-office header to true' do
        expect { filter(mail_hash) }
          .to change { mail_hash[:'x-devsquest-out-of-office'] }.to(true)
      end
    end
