  ...
  context 'authenticated as admin' do
    ...
    context 'for Facebook' do
      ...
      describe '#app_verify' do
        describe 'failure cases' do
          context 'when permission for Facebook channel is deactivated' do
            ...
            it 'returns 403 Forbidden with internal (devsquest) error' do
              post '/api/v1/external_credentials/facebook/app_verify', as: :json
              expect(response).to have_http_status(:forbidden)
          ...
          context 'with no credentials' do
            it 'returns 200 with internal (devsquest) error' do
              post '/api/v1/external_credentials/facebook/app_verify', as: :json
      ...
      describe '#link_account' do
        describe 'failure cases' do
          context 'with no credentials' do
            it 'returns 422 unprocessable entity with internal (devsquest) error' do
              get '/api/v1/external_credentials/facebook/link_account', as: :json
    ...
    context 'for Twitter', :use_vcr, required_envs: %w[TWITTER_CONSUMER_KEY TWITTER_CONSUMER_SECRET TWITTER_OAUTH_TOKEN TWITTER_OAUTH_TOKEN_SECRET TWITTER_DEV_ENVIRONMENT] do
      shared_context 'for callback URL configuration' do
        ...
        before do
          ...
          Setting.set('fqdn', 'devsquest.example.com')
        end
      end
      ...
      describe 'POST /api/v1/external_credentials/twitter/app_verify' do
        ...
        context 'with valid credential params, callback URL, and dev env label' do
          ...
          context 'with an existing, invalid webhook registered to devsquest' do
            include_examples 'for successful webhook connection'
          ...
          context 'with an existing, valid webhook registered to devsquest' do
            include_examples 'for successful webhook connection'
      ...
      describe 'GET /api/v1/external_credentials/twitter/callback' do
        ...
        #
        # 1. User clicks "Add Twitter account" in devsquest.
        #    devsquest asks Twitter for request token, saves it to session,
        ...
        # 3. devsquest asks Twitter for an access token
        ...
        # devsquest sends these parameters back to Twitter for validation.
