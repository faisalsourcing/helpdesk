...
RSpec.describe 'Organization', type: :request, searchindex: true do
  ...
  describe 'request handling' do
    ...
    it 'does index with agent' do
      ...
      get "/api/v1/organizations/search?query=#{CGI.escape('devsquest')}", params: {}, as: :json
      expect(response).to have_http_status(:ok)
      expect(json_response).to be_a_kind_of(Array)
      organization = json_response.detect { |object| object['name'] == 'devsquest Foundation' }
      expect(organization['name']).to eq('devsquest Foundation')
      expect(organization['member_ids']).to be_truthy
      expect(organization['members']).to be_falsey

      get "/api/v1/organizations/search?query=#{CGI.escape('devsquest')}&expand=true", params: {}, as: :json
      expect(response).to have_http_status(:ok)
      expect(json_response).to be_a_kind_of(Array)
      organization = json_response.detect { |object| object['name'] == 'devsquest Foundation' }
      expect(organization['name']).to eq('devsquest Foundation')
      ...
      get "/api/v1/organizations/search?query=#{CGI.escape('devsquest')}&label=true", params: {}, as: :json
      expect(response).to have_http_status(:ok)
      expect(json_response).to be_a_kind_of(Array)
      organization = json_response.detect { |object| object['label'] == 'devsquest Foundation' }
      expect(organization['label']).to eq('devsquest Foundation')
      expect(organization['value']).to eq('devsquest Foundation')
