...
RSpec.describe 'Manage > Settings > System > Network', type: :request, required_envs: %w[devsquest_PROXY_USERNAME devsquest_PROXY_PASSWORD] do

  let(:group) { create(:group) }
  let!(:admin) do
    create(:admin, groups: [Group.lookup(name: 'Users'), group])
  end
  let(:proxy) { ENV['devsquest_PROXY'] }
  let(:proxy_username) { ENV['devsquest_PROXY_USERNAME'] }
  let(:proxy_password) { ENV['devsquest_PROXY_PASSWORD'] }
  let(:valid_params) do
    {
      proxy:          proxy,
