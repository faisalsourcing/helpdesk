...
RSpec.describe 'Chat Handling', type: :system do
  ...
  def open_chat_dialog
    expect(page).to have_css('.devsquest-chat')
    click '.devsquest-chat .js-chat-open'
    expect(page).to have_css('.devsquest-chat-is-shown')
  end

  def send_customer_message(message)
    find('.devsquest-chat .devsquest-chat-input').send_keys(message)
    click '.devsquest-chat .devsquest-chat-send'
  end
  ...
  shared_examples 'chat button is hidden after idle timeout' do
    it 'check that button is hidden after idle timeout', authenticated_as: :authenticate do
      ...
      using_session :customer do
        ...
        expect(page).to have_css('.devsquest-chat', visible: :all)
        expect(page).to have_css('.devsquest-chat-is-hidden', visible: :all)
        expect(page).to have_no_css('.open-devsquest-chat:not([style*="display: none"]', visible: :all)
      end
    end
  end
  ...
  shared_examples 'chat messages' do
    it 'messages in each direction, starting on agent side', authenticated_as: :authenticate do
      ...
      using_session :customer do
        check_content('.devsquest-chat .devsquest-chat-agent-status', 'Online')
        check_content('.devsquest-chat', 'my name is me')
      ...
      using_session :customer do
        click '.js-chat-toggle .devsquest-chat-header-icon'
      end
    ...
    it 'messages in each direction, starting on customer side', authenticated_as: :authenticate do
      ...
      using_session :customer do
        ...
        check_content('.devsquest-chat .devsquest-chat-agent-status', 'Offline')
        check_content('.devsquest-chat', %r{(Chat closed by|Chat beendet von)})

        click '.devsquest-chat .js-chat-toggle .devsquest-chat-header-icon'

        expect(page).to have_no_css('.devsquest-chat-is-open')
  ...
  shared_examples 'open chat with button' do
    it 'open the chat', authenticated_as: :authenticate do
      ...
      using_session :customer do
        ...
        expect(page).to have_css('.devsquest-chat', visible: :all)
        expect(page).to have_css('.devsquest-chat-is-hidden', visible: :all)
        expect(page).to have_no_css('.devsquest-chat-is-shown', visible: :all)
        expect(page).to have_no_css('.devsquest-chat-is-open', visible: :all)

        click '.open-devsquest-chat'

        expect(page).to have_css('.devsquest-chat-is-shown', visible: :all)
        expect(page).to have_css('.devsquest-chat-is-open', visible: :all)
        check_content('.devsquest-chat-modal-text', %r{(waiting|Warte)})

        click '.devsquest-chat-header-icon-close'
  ...
  shared_examples 'timeouts' do
    it 'check different timeouts', authenticated_as: :authenticate do
      ...
      using_session :customer do
        ...
        expect(page).to have_no_css('.devsquest-chat')
        ...
        check_content('.devsquest-chat-modal-text', %r{(waiting|Warte)})
        check_content('.devsquest-chat-modal-text', %r{(takes longer|dauert länger)})
      ...
      using_session :customer do
        ...
        check_content('.devsquest-chat', 'agent is asking')

        check_content('.devsquest-chat-modal-text', %r{(Since you didn't respond|Da Sie in den letzten)}, wait: 30)
      end
  ...
  context 'when chat is activated or disabled' do
    it 'switch the chat setting', authenticated_as: :authenticate do
      ...
      using_session :customer do
        ...
        check_content('.devsquest-chat-modal-text', %r{(waiting|Warte)}, should_match: false)
      end
  ...
  context 'when changing chat preferences for current agent' do
    it 'use chat phrase preference', authenticated_as: :authenticate do
      ...
      using_session :customer do
        check_content('.devsquest-chat', %r{(Hi Stranger|My Greeting)})
      end
    ...
    shared_examples 'test issue #2471' do
      it 'is able to close to the dialog after a idleTimeout happened' do
        ...
        using_session :customer do
          ...
          click '.js-chat-toggle .devsquest-chat-header-icon'
          expect(page).to have_no_selector('devsquest-chat-is-open', wait: 60)
        end
      end
    end
