...
class EmailHelperTest < ActiveSupport::TestCase
  ...
  test 'a mx_records' do
    ...
    domain = 'devsquest.com'
    mx_domains = EmailHelper.mx_records(domain)
    assert_equal('mx2.devsquest.com', mx_domains[0])
  ...
  test 'provider_inbound_mx' do
    ...
    assert_equal('mx2.devsquest.com', map[0][:options][:host])
    ...
    assert_equal('mx2.devsquest.com', map[1][:options][:host])
  ...
  test 'provider_inbound_guess' do
    ...
    assert_equal('imap', map[0][:adapter])
    assert_equal('mail.devsquest.com', map[0][:options][:host])
    assert_equal(993, map[0][:options][:port])
    assert_equal(user, map[0][:options][:user])
    assert_equal(password, map[0][:options][:password])

    assert_equal('imap', map[1][:adapter])
    assert_equal('mail.devsquest.com', map[1][:options][:host])
    assert_equal(993, map[1][:options][:port])
    assert_equal(email, map[1][:options][:user])
    assert_equal(password, map[1][:options][:password])
  ...
  test 'z probe_inbound' do
    ...
    assert_equal('mx2.devsquest.com', result[:settings][:options][:host])

    # realtest - test I
    if !ENV['EMAILHELPER_MAILBOX_1']
      raise "Need EMAILHELPER_MAILBOX_1 as ENV variable like export EMAILHELPER_MAILBOX_1='unittestemailhelper01@devsquest.com:somepass'"
    end
  ...
  test 'zz probe' do
    ...
    assert_equal('ok', result[:result])
    assert_equal('mx2.devsquest.com', result[:setting][:inbound][:options][:host])
    assert_equal('mx2.devsquest.com', result[:setting][:outbound][:options][:host])
