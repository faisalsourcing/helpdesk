...
class EmailPostmasterToSender < ActiveSupport::TestCase
  ...
  setup do
    ...
    # setup the IMAP account info for devsquest
    ...
    @email_address = EmailAddress.create!(
      ...
      email:         "some-devsquest-#{ENV['MAIL_SERVER_EMAIL']}",
      updated_by_id: 1,
      created_by_id: 1,
    )
...
Message-ID: <#{@test_id}@devsquest.test.com>
...
".gsub(%r{\n}, "\r\n")
    ...
    assert_equal("<#{@test_id}@devsquest.test.com>",
                 mail['references'],
    ...
    assert(body.include?(Setting.get('fqdn')), 'Body must contain the devsquest instance name')
