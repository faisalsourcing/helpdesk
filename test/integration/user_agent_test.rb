...
class UserAgentTest < ActiveSupport::TestCase
  host = 'https://web-test.dc.devsquest.com'
  # host = 'http://127.0.0.1:3003'

  setup do
    if ENV['devsquest_PROXY_TEST'] == 'true'
      Setting.set('proxy', ENV['devsquest_PROXY'])
      Setting.set('proxy_username', ENV['devsquest_PROXY_USERNAME'])
      Setting.set('proxy_password', ENV['devsquest_PROXY_PASSWORD'])
    end
  end
  ...
  test 'check some results' do
    ...
    if ENV['devsquest_PROXY_TEST'] == 'true' && ENV['devsquest_PROXY_REMOTE_IP_CHECK']
      assert_match(%r{"remote_ip":"#{ENV['devsquest_PROXY_REMOTE_IP_CHECK']}"}, result.body)
    end
